...
function set-keystore-infinispan-server() {
  sed 's/<security-realm name="default">/<security-realm name="default"><server-identities><ssl><keystore path="/keystore.p12" relative-to="infinispan.server.config.path" password="" \/><\/ssl><\/server-identities>/' -i ./infinispan-config.xml
}
